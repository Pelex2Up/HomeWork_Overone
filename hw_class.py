    ...
    def func1(self, some_string):
        if not some_string.isdigit():
            for i in some_string:
                if i not in self.empty_gl and i in self.glasn:
                    self.empty_gl.append(i)
                elif i not in self.empty_sogl:
            ...
            if result <= self.func2(some_string):
                return self.empty_gl
            else:
                return self.empty_sogl
        else:
            for i in some_string:
                if int(i) % 2 == 0:
                    self.sum_chet += int(i)
                    ...
                    continue
            return self.sum_chet * self.func2(some_string)
